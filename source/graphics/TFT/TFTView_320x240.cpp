...
void TFTView_320x240::ui_set_active(lv_obj_t *b, lv_obj_t *p, lv_obj_t *tp)
{
    ...
    } else if (inputdriver->hasEncoderDevice()) {
        setGroupFocus(activePanel);
    }

    // Removed: lv_obj_add_flag(objects.keyboard, LV_OBJ_FLAG_HIDDEN);
...
void TFTView_320x240::apply_hotfix(void)
{
    ...
    if (v >= 480) {
        // keyboard size limit - REMOVED: Virtual keyboard not needed for T-Deck
    ...
    // Removed: lv_obj_move_foreground(objects.keyboard);
...
void TFTView_320x240::ui_events_init(void)
{
    ...
    // keyboard - REMOVED: Virtual keyboard not needed for T-Deck
    // Removed all keyboard event callbacks
...
void TFTView_320x240::ui_event_KeyboardButton(lv_event_t *e)
{
    ...
    /**
     * handle events for virtual keyboard - REMOVED: Not needed for T-Deck
     */
    void TFTView_320x240::ui_event_Keyboard(lv_event_t * e)
    {
        ...
        void TFTView_320x240::ui_event_preset_button(lv_event_t * e)
        {
            ...
            if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone && THIS->db.config.lora.use_preset) {
                ...
                lv_dropdown_set_selected(objects.settings_modem_preset_dropdown,
                                         THIS->preset2val(THIS->db.config.lora.modem_preset));
                ...
                uint32_t numChannels =
                    LoRaPresets::getNumChannels(THIS->db.config.lora.region, THIS->db.config.lora.modem_preset);
        ...
        void TFTView_320x240::ui_event_input_button(lv_event_t * e)
        {
            ...
            if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
                ...
                lv_dropdown_get_selected_str(objects.settings_keyboard_input_dropdown, THIS->old_val1_scratch,
                                             sizeof(old_val1_scratch));
                lv_dropdown_get_selected_str(objects.settings_mouse_input_dropdown, THIS->old_val2_scratch,
        ...
        void TFTView_320x240::ui_event_alert_button(lv_event_t * e)
        {
            lv_event_code_t event_code = lv_event_get_code(e);
            if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone &&
                THIS->db.module_config.has_external_notification) {
            ...
            if (event_code == LV_EVENT_CLICKED) {
                ...
                if (position.fixed_position) {
                    ...
                    if (position.gps_mode != meshtastic_Config_PositionConfig_GpsMode_NOT_PRESENT) {
                        ...
                        THIS->controller->sendConfig(
                            meshtastic_Position{.latitude_i = ilat,
                                                .longitude_i = ilon,
            ...
            if (!map) {
                ...
                if (hasPosition) {
                    if (db.uiConfig.map_data.has_home) {
                        map->setHomeLocation(db.uiConfig.map_data.home.latitude * 1e-7,
        ...
        void TFTView_320x240::packetDetected(const meshtastic_MeshPacket &p)
        {
            ...
            if (heard) {
                if (p.to == ownNode && p.decoded.portnum == meshtastic_PortNum_NODEINFO_APP) {
                    ...
                    setNodeImage(p.from, (MeshtasticView::eRole)(unsigned long)nodes[p.from]->LV_OBJ_IDX(node_img_idx)->user_data,
        ...
        void TFTView_320x240::writePacketLog(const meshtastic_MeshPacket &p)
        {
            ...
            if (p.to == 0xffffffff)
                ...
                sprintf(buf, "%s: ch%d %s:%04x->%s%04x: %s", timebuf, p.channel, from, p.from & 0xffff,
                        p.to == ownNode ? "*" : "", p.to & 0xffff, name[p.decoded.portnum]);

            if (p.decoded.portnum == meshtastic_PortNum_TELEMETRY_APP) {
                meshtastic_Telemetry telemetry;
                if (pb_decode_from_bytes(p.decoded.payload.bytes, p.decoded.payload.size, &meshtastic_Telemetry_msg,
                                         &telemetry)) {
                ...
                if (pb_decode_from_bytes(p.decoded.payload.bytes, p.decoded.payload.size, &meshtastic_RouteDiscovery_msg,
                                         &route)) {
        ...
        void TFTView_320x240::ui_event_ok(lv_event_t * e)
        {
            ...
            if (event_code == LV_EVENT_CLICKED) {
                ...
                case eSetup: {
                    ...
                    if (region != THIS->db.config.lora.region) {
                        ...
                        uint32_t defaultSlot =
                            lora.region == meshtastic_Config_LoRaConfig_RegionCode_UNSET ? lora.channel_num : 0;
                        if (defaultSlot == 0) {
                ...
                case eWifi: {
                    ...
                    if (strcmp(THIS->db.config.network.wifi_ssid, ssid) != 0 ||
                        strcmp(THIS->db.config.network.wifi_psk, psk) != 0) {
                ...
                case eInputControl: {
                    ...
                    std::string current_kbd = THIS->inputdriver->getCurrentKeyboardDevice();
                    std::string current_ptr = THIS->inputdriver->getCurrentPointerDevice();
                    if (strcmp(current_kbd.c_str(), _("none")) == 0 && strcmp(current_ptr.c_str(), _("none")) == 0 &&
                        THIS->input_group) {
                ...
                case eModifyChannel: {
                    ...
                    if (strlen(base64) == 0 && strlen(name) == 0) {
                        ...
                        memset(THIS->channel_scratch[ch].settings.psk.bytes, 0,
                               sizeof(THIS->channel_scratch[ch].settings.psk.bytes));
                        ...
                        if (THIS->base64ToPsk(lv_textarea_get_text(objects.settings_modify_channel_psk_textarea), psk.bytes,
                                              psk.size)) {
                        ...
                        THIS->channel_scratch[ch].role =
                            (ch == 0) ? meshtastic_Channel_Role_PRIMARY : meshtastic_Channel_Role_SECONDARY;
                    }
        ...
        void TFTView_320x240::ui_event_modem_preset_dropdown(lv_event_t * e)
        {
            ...
            uint32_t channel =
                LoRaPresets::getDefaultSlot(THIS->db.config.lora.region, preset, THIS->db.channel[0].settings.name);
        ...
        void TFTView_320x240::addNode(uint32_t nodeNum, uint8_t ch, const char *userShort, const char *userLong,
                                      uint32_t lastHeard, eRole role, bool hasKey, bool unmessagable)
        {
            ...
            ILOG_DEBUG("addNode(%d): num=0x%08x, lastseen=%d, name=%s(%s), role=%d", nodeCount, nodeNum, lastHeard, userLong,
                       userShort, role);
        ...
        void TFTView_320x240::setDeviceMetaData(int hw_model, const char *version, bool has_bluetooth, bool has_wifi,
         ...
         * @param viaMqtt
         */
        // void TFTView_320x240::updateNode(uint32_t nodeNum, uint8_t ch, const char *userShort, const char *userLong, uint32_t
        // lastHeard,
        ...
        void TFTView_320x240::updateNode(uint32_t nodeNum, uint8_t ch, const meshtastic_User &cfg)
        {
            ...
            if (it != nodes.end() && it->second) {
                ...
                if (cfg.public_key.size != 0) {
                    ...
                    lv_color_t color =
                        lv_obj_get_style_bg_color(it->second->LV_OBJ_IDX(node_img_idx), LV_PART_MAIN | LV_STATE_DEFAULT);
        ...
        void TFTView_320x240::updatePosition(uint32_t nodeNum, int32_t lat, int32_t lon, int32_t alt, uint32_t sats,
                                             uint32_t precision)
        {
            ...
            if (nodeNum == ownNode) {
                ...
                if (sats)
                    sprintf(buf, "%c%02i° %2i'%02i\"   %u sats\n%c%02i° %2i'%02i\"   %d%s", latLetter, abs(latDegrees),
                            latMinutes, latSeconds, sats, lonLetter, abs(lonDegrees), lonMinutes, lonSeconds, altU, units);
                else
                    sprintf(buf, "%c%02i° %2i'%02i\"\n%c%02i° %2i'%02i\"   %d%s", latLetter, abs(latDegrees), latMinutes,
                            latSeconds, lonLetter, abs(lonDegrees), lonMinutes, lonSeconds, altU, units);
            ...
            if (status.has_wifi) {
                if (db.config.network.wifi_enabled || db.config.network.eth_enabled) {
                    if (status.wifi.has_status) {
                        ...
                        sprintf(buf, "%d.%d.%d.%d", ip & 0xff, (ip & 0xff00) >> 8, (ip & 0xff0000) >> 16,
                                (ip & 0xff000000) >> 24);
        ...
        void TFTView_320x240::onTextMessageCallback(const ResponseHandler::Request &req, ResponseHandler::EventType evt,
                                                    int32_t result)
        {
            ...
            {
                {
                    ...
                    if (nodePanel) {
                        setNodeImage(nodeNum,
                                     (MeshtasticView::eRole)(unsigned long)nodePanel->LV_OBJ_IDX(node_img_idx)->user_data, false,
                                     img);
                    } else {
        ...
        void TFTView_320x240::updateChannelConfig(const meshtastic_Channel &ch)
        {
            ...
            if (ch.role != meshtastic_Channel_Role_DISABLED) {
                ...
                } else if (memcmp(ch.settings.psk.bytes, "\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000",
                                  16) == 0) {
        ...
        void TFTView_320x240::updateLoRaConfig(const meshtastic_Config_LoRaConfig &cfg)
        {
            ...
            if (cfg.use_preset) {
                // This must be run before displaying LoRa frequency as channel of 0 ("calculate from hash") leads to an integer
                // underflow
                if (!db.config.lora.channel_num) {
                    db.config.lora.channel_num = LoRaPresets::getDefaultSlot(
                        db.config.lora.region, THIS->db.config.lora.modem_preset, THIS->db.channel[0].settings.name);
                }
        ...
        void TFTView_320x240::updateExtNotificationModule(const meshtastic_ModuleConfig_ExternalNotificationConfig &cfg)
        {
            ...
            lv_snprintf(buf, sizeof(buf), _("Message Alert: %s"),
                        db.module_config.external_notification.alert_message_buzzer &&
                                db.module_config.external_notification.enabled
                            ? _("on")
                            : _("off"));
            lv_label_set_text(objects.basic_settings_alert_label, buf);
        }
        ...
        lv_obj_t *TFTView_320x240::newMessageContainer(uint32_t from, uint32_t to, uint8_t ch)
        {
            ...
            lv_obj_clear_flag(container,
                              lv_obj_flag_t(LV_OBJ_FLAG_PRESS_LOCK | LV_OBJ_FLAG_CLICK_FOCUSABLE | LV_OBJ_FLAG_GESTURE_BUBBLE |
        ...
        void TFTView_320x240::restoreMessage(const LogMessage &msg)
        {
            ...
            // ILOG_DEBUG("restoring msg from:0x%08x, to:0x%08x, ch:%d, time:%d, status:%d, trash:%d, size:%d, '%s'", msg.from,
            // msg.to,
        ...
        void TFTView_320x240::updateFreeMem(void)
        {
            ...
            if (activePanel == objects.home_panel && (unsigned long)objects.home_memory_button->user_data) {
                ...
                sprintf(buf, _("Heap: %d (%d%%)\nLVGL: %d (%d%%)"), freeHeap, freeHeap_pct, mon.free_size / 1024,
                        100 - mon.used_pct);
#else
                buf[0] = '\0';
#endif
            ...
            if (screensInitialised) {
                ...
                if (curtime - lastrun1 >= 1) { // call every 1s
                    ...
                    if (curtime - lastrun10 >= 10) { // call every 10s
                        ...
                        if ((db.config.network.wifi_enabled || db.module_config.mqtt.enabled) &&
                            !displaydriver->isPowersaving()) {
