...
LVGLGraphics::LVGLGraphics(uint16_t width, uint16_t height)
    : screenWidth(width), screenHeight(height) {

#if defined(USE_DOUBLE_BUFFER) // speedup drawing by using double-buffered DMA
                               // mode
  ...
  buf1 = (lv_color_t *)heap_caps_aligned_alloc(32, (bufsize + 3) & ~3,
                                               MALLOC_CAP_SPIRAM);
  // buf2 = (lv_color_t*)heap_caps_aligned_alloc(16, (bufsize + 3) & ~3,
  // MALLOC_CAP_SPIRAM);
  draw_buf = (lv_disp_draw_buf_t *)heap_caps_aligned_alloc(
      32, sizeof(lv_disp_draw_buf_t), MALLOC_CAP_SPIRAM);
#else
  buf1 = (lv_color_t *)heap_caps_malloc(
      bufsize, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA); // heap_alloc_dma
  buf2 = (lv_color_t *)heap_caps_malloc(
      bufsize, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA); // heap_alloc_dma
  draw_buf = (lv_disp_draw_buf_t *)heap_caps_malloc(
      sizeof(lv_disp_draw_buf_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA);
#endif
  ...
  bufsize = width * height / 8 * sizeof(lv_color_t);
  buf1 = (lv_color_t *)heap_caps_malloc(
      bufsize, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT); // heap_alloc_psram
  draw_buf = (lv_disp_draw_buf_t *)heap_caps_aligned_alloc(
      16, sizeof(lv_disp_draw_buf_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  assert(buf1 != 0);
#else
  bufsize = width * 10;
...
void LVGLGraphics::init(void) {
  ...
  lv_log_register_print_cb(
      lv_debug); /* register print function for debugging */
#endif

  /* LVGL : Setting up buffer to use for display */
  Serial.print("LV drawbuf size=");
