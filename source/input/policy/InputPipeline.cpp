...
bool InputPipeline::process(InputEvent &event, const InputCapabilities &capabilities, std::vector<InputEvent> &outEvents)
{
    ...
    if (bindingResolver) {
        ...
        if (oldAction != event.action) {
            POLICY_DEBUG("[Pipeline] Binding resolved: %d -> %d", (int)oldAction, (int)event.action);
        }
    }

    POLICY_DEBUG("[Pipeline] Evaluating policy chain with %d policies", chain.getPolicyCount());
    PolicyDecision decision = chain.evaluate(event, context, capabilities);
    POLICY_DEBUG("[Pipeline] Policy chain decision: type=%d", (int)decision.type);

    switch (decision.type) {
    case DecisionType::Pass:
        POLICY_DEBUG("[Pipeline] Decision=Pass, forwarding event 0x%x", event.rawKeyCode);
    ...
    case DecisionType::Consume:
        POLICY_DEBUG("[Pipeline] Decision=Consume, dropping event 0x%x", event.rawKeyCode);
        return false;
    case DecisionType::EmitCommand:
        POLICY_DEBUG("[Pipeline] Decision=EmitCommand, cmd=%d", (int)decision.command.command);
    ...
    case DecisionType::EmitSequence:
        POLICY_DEBUG("[Pipeline] Decision=EmitSequence, %d events", (int)decision.emittedEvents.size());
