...
// removed all exception code and replaced with ILOG_ERROR
...
inline headerv3 deserialize_header(const std::string &s)
{
    ...
    if (s.size() != sizeof(headerv3) || s[7] != 0x3) {
        ILOG_ERROR("Invalid .pmtiles header");
        return headerv3{};
    }
    headerv3 h;
                                              ...
                                              const char *pmtiles_map, uint8_t z, uint32_t x, uint32_t y)
{
    ...
    if (h.root_dir_bytes > std::numeric_limits<uint32_t>::max()) {
        ILOG_ERROR("malformed directory 8");
        return {};
    }
