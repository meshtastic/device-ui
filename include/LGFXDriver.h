...
template <class LGFX>
void LGFXDriver<LGFX>::display_flush(lv_disp_drv_t *disp, const lv_area_t *area,
                                     lv_color_t *color_p) {
...
template <class LGFX>
void LGFXDriver<LGFX>::touchpad_read(lv_indev_drv_t *indev_driver,
                                     lv_indev_data_t *data) {
...
template <class LGFX> void LGFXDriver<LGFX>::init_lgfx(void) {
  ...
  TFTDriver<LGFX>::tft->drawString("touch the arrow marker.", tft->width() >> 1,
                                   tft->height() >> 1);
  ...
  if (TFTDriver<LGFX>::tft->isEPD())
    std::swap(fg, bg);
  TFTDriver<LGFX>::tft->calibrateTouch(
      parameters, fg, bg, std::max(tft->width(), tft->height()) >> 3);
