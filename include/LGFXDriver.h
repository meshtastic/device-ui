...
template <class LGFX>
class LGFXDriver : public TFTDriver<LGFX> {
public:
...
template <class LGFX>
LGFX* LGFXDriver<LGFX>::lgfx = nullptr;


template <class LGFX>
LGFXDriver<LGFX>::LGFXDriver(uint16_t width, uint16_t height) 
  : TFTDriver<LGFX> (lgfx ? lgfx : new LGFX, width, height) {
    lgfx = this->tft;
}
...
template <class LGFX>
void LGFXDriver<LGFX>::touchpad_read(lv_indev_drv_t *indev_driver, lv_indev_data_t *data) {
...
template <class LGFX>
void LGFXDriver<LGFX>::init(void) {
    ...
    //disp_drv.full_refresh = 1; //needs allocate entire draw_buf width*height in PSRAM
#endif
...
template <class LGFX>
void LGFXDriver<LGFX>::init_lgfx(void) {
    ...
    if (TFTDriver<LGFX>::tft->isEPD()) std::swap(fg, bg);
