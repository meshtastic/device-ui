...
void init_style_main_button_style_MAIN_DEFAULT(lv_style_t *style) {
    ...
    lv_style_set_bg_image_recolor(style, lv_color_hex(0xffc0c0c0));
...
void init_style_main_button_style_MAIN_FOCUSED(lv_style_t *style) {
    ...
    lv_style_set_transform_width(style, 10);
    lv_style_set_transform_height(style, 10);
    lv_style_set_transform_scale_x(style, 270);
    lv_style_set_transform_scale_y(style, 270);
    lv_style_set_bg_image_recolor(style, lv_color_hex(0xffffffff));
...
void init_style_top_panel_style_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_bg_color(style, lv_color_hex(0xff3e3e3e));
    lv_style_set_text_font(style, &lv_font_montserrat_20);
    ...
    lv_style_set_text_color(style, lv_color_hex(0xffa6f0ff));
    lv_style_set_border_color(style, lv_color_hex(0xff707070));
};
...
void init_style_home_button_style_MAIN_FOCUSED(lv_style_t *style) {
    lv_style_set_align(style, LV_ALIGN_TOP_MID);
};

lv_style_t *get_style_home_button_style_MAIN_FOCUSED() {
    static lv_style_t *style;
    if (!style) {
        style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_home_button_style_MAIN_FOCUSED(style);
    }
    return style;
};
...
void add_style_home_button_style(lv_obj_t *obj) {
    ...
    lv_obj_add_style(obj, get_style_home_button_style_MAIN_FOCUSED(), LV_PART_MAIN | LV_STATE_FOCUSED);
};
...
void remove_style_home_button_style(lv_obj_t *obj) {
    ...
    lv_obj_remove_style(obj, get_style_home_button_style_MAIN_FOCUSED(), LV_PART_MAIN | LV_STATE_FOCUSED);
};
