...
LVGLGraphics::LVGLGraphics(uint16_t width, uint16_t height) :
    ...
    //buf1 = (lv_color_t*)heap_caps_malloc(bufsize, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA);  //assert failed: block_trim_free heap_tlsf.c:371
    //buf2 = (lv_color_t*)heap_caps_malloc(bufsize, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA);
    //buf1 = (lv_color_t*)heap_caps_malloc((bufsize + 3) & ~3, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT); // crash
    //buf1 = (lv_color_t*)heap_caps_malloc(bufsize, MALLOC_CAP_SPIRAM); // crash
    //buf1 = (lv_color_t*)ps_malloc(bufsize); // crash
    buf1 = (lv_color_t*)heap_caps_aligned_alloc(32, (bufsize + 3) & ~3, MALLOC_CAP_SPIRAM);
    //buf2 = (lv_color_t*)heap_caps_aligned_alloc(16, (bufsize + 3) & ~3, MALLOC_CAP_SPIRAM);
...
void LVGLGraphics::init(void) {
    ...
    Serial.print("LV drawbuf size="); Serial.println(bufsize);
