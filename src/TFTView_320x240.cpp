...
#ifdef USE_X11
TFTView_320x240::TFTView_320x240() : DeviceGUI(&X11Driver::create(), new ViewController) {
}
#else
...
void TFTView_320x240::ui_events_init(void) {
    ...
    lv_obj_add_event_cb(ui_NodeButton1, ui_event_NodeButtonClicked, LV_EVENT_ALL, ui_NodeLabel1);
    lv_obj_add_event_cb(ui_NodeButton2, ui_event_NodeButtonClicked, LV_EVENT_ALL, ui_UserNameLabel2);
    lv_obj_add_event_cb(ui_NodeButton3, ui_event_NodeButtonClicked, LV_EVENT_ALL, ui_UserNameLabel3);
...
void TFTView_320x240::addMessage(char* msg) {
    ...
    lv_obj_set_height(textLabel, LV_SIZE_CONTENT);    /// 1
    ...
    Serial.print("msg lenght=");
    Serial.println(strlen(msg));
    int16_t pos = lv_obj_get_scroll_y(hiddenPanel);
    Serial.print("lv_obj_get_scroll_y=");
    Serial.print(pos);
    pos = lv_obj_get_scroll_top(hiddenPanel);
    Serial.print(", lv_obj_get_scroll_top=");
    Serial.print(pos);
    pos = lv_obj_get_scroll_bottom(hiddenPanel);
    Serial.print(", lv_obj_get_scroll_bottom=");
    Serial.println(pos);
...
void TFTView_320x240::populate_nodes(void) {
    static lv_obj_t *node_panel[100];
    srand(time(0));
    for (int i=0; i<99; i++) {
        uint32_t r = rand(); 
        char numbuf[5], namebuf[20] = "Meshtastic "; 
        std::sprintf(numbuf, "%04x\0", uint16_t(r));
        ...
        uint32_t red = (r & 0xff0000) >> 16;
        uint32_t green = (r & 0xff00) >> 8;
        uint32_t blue = (r & 0xff);
        ...
        lv_obj_set_height( img, LV_SIZE_CONTENT);   /// 1
        lv_obj_set_x( img, -5 );
        lv_obj_set_y( img, -10 );
        ...
        lv_obj_set_width( ln_lbl, LV_SIZE_CONTENT);  /// 1
        lv_obj_set_height( ln_lbl, LV_SIZE_CONTENT);   /// 1
        lv_obj_set_x( ln_lbl, -6 );
        lv_obj_set_y( ln_lbl, 12 );
        lv_obj_set_align( ln_lbl, LV_ALIGN_BOTTOM_LEFT );
        lv_label_set_text(ln_lbl, namebuf);
        ...
        lv_obj_set_width( sn_lbl, LV_SIZE_CONTENT);  /// 1
        lv_obj_set_height( sn_lbl, LV_SIZE_CONTENT);   /// 1
        lv_obj_set_x( sn_lbl, 25 );
        lv_obj_set_y( sn_lbl, -3 );
        lv_obj_set_align( sn_lbl, LV_ALIGN_LEFT_MID );
        lv_label_set_text(sn_lbl, numbuf);
        ...
        lv_obj_set_x(ui_BatteryLabel, lv_pct(0));
        lv_obj_set_align(ui_BatteryLabel, LV_ALIGN_RIGHT_MID);
        lv_label_set_text(ui_BatteryLabel, "65% 3.68V");
        ...
        lv_label_set_text(ui_lastHeardLabel, "15 min");
